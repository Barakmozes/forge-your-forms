# DUAL-AUTOMATION-PROMPT V4 — Complete Development Lifecycle Automation
...
## Mechanic 1: Three-File Architecture (V4)

**What**: The system splits into 3 files instead of 1 monolith.

| File | Purpose | Token Load |
|------|---------|-----------|
| `DUAL-AUTOMATION-PROMPT-V4.md` | Core Scanner + Builder prompts | ~12K tokens |
...
**Stack-specific addons**: V4 includes security audit commands per ecosystem (cargo audit, npm audit, pip-audit, etc.) ensuring cross-language parity.
...
## Mechanic 8: Runtime Performance Dimension (V4)

V4 includes deep performance scanning beyond V2's bundler-only check:
...
## Mechanic 9: Responsive Audit (V4)

V4 includes modern CSS features missing from V2:
...
## Mechanic 11: Orchestrator Sync (V4)

The V4 orchestrator (`orchestrator.mjs`) is fully synced with the 3-file architecture:

**3-File Extraction**: Orchestrator prepends file-read instructions when sending prompts to `claude -p`:
- Scanner sessions receive: "Read SCAN-DIMENSIONS.md before starting scan work"
- Builder sessions receive: "Verify AGENT-TEMPLATES.md exists before proceeding"
- File existence checks run before each phase — missing files produce clear errors, not crashes

**Quality Gates**: Automated checks run between batch groups:
- Gate after batch 1 (Post-Infrastructure): lint + typecheck
- Gate after batch 3 (Post-Bugfix): build
- Gate after batch 5 (Pre-Feature): build + test
- Gate after batch 6 (Post-Feature): build + test
- Failed gates attempt auto-fix via `claude -p`, then pause with resume/skip options
- State persisted to `.orchestrator/state.json` for resume across sessions

**10 Role Colors in Dashboard**: Each agent displays its role type with ANSI color coding:
- ENGINEER (green), SECURITY_ENGINEER (red), RESPONSIVE_SPECIALIST (cyan)
- PERFORMANCE_ENGINEER (yellow), DEVOPS_ENGINEER (yellow), PRODUCT_BUILDER (magenta)
- ARCHITECT (blue), DOCS_WRITER (default), ROADMAP_COMPILER (magenta), VERIFIER (bold green)

**9 Batch Tiers**: Dynamic batch count parsed from COMMANDS.md — no hardcoded limits

**Project Type Display**: Dashboard header shows project type (Web/CLI/Library/Mobile/Backend/Service) and active dimension count from PROJECT-PROFILE.md

**Completion Signals**: `SCANNER_COMPLETE`, `BUILDER_COMPLETE`, `AGENT_NN_COMPLETE`, `CONTEXT_LIMIT_REACHED`, `GATE_PASSED_BATCH_N`, `GATE_FAILED_BATCH_N`

**Enhanced Summary**: Pipeline completion shows per-role agent counts, gate results, product roadmap metrics, and verification status
...
# End of DUAL-AUTOMATION-PROMPT V4
